<!--
delete.php파일에서 '삭제'버튼을 누르면 실행되는 파일로 db에 저장된 사건의 데이터를 삭제 및 갱신합니다.
-->
...
<?
  ...
  // 삭제하려고 선택된 checkbox가 없는 경우에 삭제할 정보를 선택하라고 알려줍니다.
  if($_POST['checkbox'] == 0){
  ...
  for($i = 0; $i < $num; $i++){
    ...
    //site_info 테이블의 튜플부터 삭제합니다. (자식 테이블)
    ...
    //victim_info 테이블의 튜플을 삭제합니다. (자식 테이블)
    ...
    //cheater_info 테이블을 갱신합니다.
    //총사기횟수가 1이면 해당 사기꾼의 정보를 삭제하고,
    //총사기횟수가 2이상이면 총사기횟수와 총사기금액을 갱신해줍니다.

    //총사기횟수(cheat_count)를 받아옵니다.
    ...
    if($cheat_count > 1){ //총사기횟수가 2이상이면 총사기횟수와 사기금액을 갱신해줍니다.
      ...
      //cheater_info에서 사기꾼의 정보를 삭제합니다. (부모 테이블)
